...
class PlantWikipediaDetailScreen extends StatefulWidget {
  final ElementNature plant;

  const PlantWikipediaDetailScreen({Key? key, required this.plant})
      : super(key: key);

  @override
  State<PlantWikipediaDetailScreen> createState() =>
      _PlantWikipediaDetailScreenState();
}

class _PlantWikipediaDetailScreenState
    extends State<PlantWikipediaDetailScreen> {
  ...
  Future<void> _loadWikipediaData() async {
    ...
    try {
      print('=== DEBUG WIKIPEDIA PLANT ===');
      print('plant.name: "${widget.plant.name}"');
      print('plant.wikipediaSource: "${widget.plant.wikipediaSource}"');
      print('plant.wikipediaSource est vide? ${widget.plant.wikipediaSource.isEmpty}');

      // Utiliser wikipediaSource de la plante si disponible, sinon utiliser le nom
      String searchTerm = widget.plant.wikipediaSource.isNotEmpty
          ? widget.plant.wikipediaSource
          : widget.plant.name;
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                // Titre
                Text(
                  wikiData?['title'] ?? widget.plant.name,
